...
io.on('connection', function(client) {
	...
	client.on('joinGame', function(tank){
		...
		client.emit('addTank', { spawn: tank.spawn, id: tank.id, type: tank.type, isLocal: true, x: initX, y: initY, hp: TANK_INIT_HP });
	...
	client.on('chatMessage', function(data){
		...
		client.emit('chatMessage', {user: data.user, msg: data.msg});
		client.broadcast.emit('chatMessage', {user: data.user, msg: data.msg});
	});
	...
	client.on('leaveGame', function(tankId){
		...
		client.broadcast.emit('disconnectedLog', tankId);
