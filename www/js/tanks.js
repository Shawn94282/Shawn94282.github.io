...
Game.prototype = {
	...
	addTank: function(id, type, isLocal, x, y, hp){
		...
		if(id != '') { alertify.warning(id + " joined the game", 6); }
		if(isLocal){
			this.localTank = t;
		}else{
			this.tanks.push(t);
	...
	disconnectedLog: function(tankId){
		if(tankId != '') {
			alertify.error(tankId + " has left the game", 6);
		}
	},

	chatMessage: function(user, msg){
		alertify.message("<strong>"+ user + "</strong>: " + msg, 10).dismissOthers();
	},
	...
	killTank: function(tank){
		tank.dead = true;
		alertify.error(tank.id + " was killed", 6);
...
Tank.prototype = {
	...
	materialize: function(){
		...
		this.$info.append('<div class="hp-bar1"><div class="hp-bar"></div></div>');
